...
struct op_bool
    : sor<op_eq, op_ne, op_lte, op_gte, op_gt, op_lt, op_and, op_or, op_regex, op_assign> {};
...
template <char Q> struct str_impl : if_must<one<Q>, until<one<Q>, char_value>> {};
struct str_lit : sor<str_impl<'\''>, str_impl<'"'>> {};

struct bool_lit
    : seq<sor<string<'t', 'r', 'u', 'e'>, string<'f', 'a', 'l', 's', 'e'>>, not_at<ident_other>> {};
...
struct to_num_stmt : seq<opt<bang>, to_num, oparan, list_must<ident, comma, sp>, cparan> {};

template <> struct control<to_num_stmt> : normal<to_num_stmt> {
    template <typename Input> static void start(const Input & /*unused*/, engine::engine &e) {
        e.new_stmt<engine::to_num_stmt>(); // TODO: fix
    }
    template <typename Input> static void success(const Input & /*unused*/, engine::engine &e) {
        e.finish_stmt();
    }
};
...
struct to_str_stmt : seq<opt<bang>, to_str, oparan, list_must<ident, comma, sp>, cparan> {};
...
struct colsstmt : seq<opt<bang>, cols, oparan, list_must<ident, comma, sp>, cparan> {};
